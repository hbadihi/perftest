...
# Usage: 
#   Server: ./atomic_multi.sh <queue_num> [start_port] [num_processes] [qp_offset_start]
#   Client: ./atomic_multi.sh <queue_num> <start_port> <num_processes> <server_host> [qp_offset_start]
...
for i in $(seq 0 $((NUM_PROCESSES - 1))); do
    ...
    if [ "$MODE" = "CLIENT" ]; then
        ...
        CMD="taskset -c $CPU ./ib_atomic_bw -q $QUEUE_NUM $QP_OFFSET_PARAM -p $PORT -t 1 --run_infinitely --out_json --out_json_file=$JSON_FILE $SERVER_HOST"
        echo "Command: $CMD" > "$LOG_FILE"
        echo "========================================" >> "$LOG_FILE"
        echo "" >> "$LOG_FILE"
        taskset -c $CPU ./ib_atomic_bw -q $QUEUE_NUM $QP_OFFSET_PARAM -p $PORT \
            -t 1 --run_infinitely --out_json --out_json_file="$JSON_FILE" \
            $SERVER_HOST >> "$LOG_FILE" 2>&1 &
    else
        # Server mode: wait for connections
        CMD="taskset -c $CPU ./ib_atomic_bw -q $QUEUE_NUM $QP_OFFSET_PARAM -p $PORT -t 1 --run_infinitely --out_json --out_json_file=$JSON_FILE"
        echo "Command: $CMD" > "$LOG_FILE"
        echo "========================================" >> "$LOG_FILE"
        echo "" >> "$LOG_FILE"
        taskset -c $CPU ./ib_atomic_bw -q $QUEUE_NUM $QP_OFFSET_PARAM -p $PORT \
